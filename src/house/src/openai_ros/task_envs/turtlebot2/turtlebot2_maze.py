...
class TurtleBot2MazeEnv(turtlebot2_env.TurtleBot2Env):
    def __init__(self, world_file_name, robot_number=0):
        ...
        self.laser_filtered_pub = rospy.Publisher('/scan', LaserScan, queue_size=1)
        self.goal_reaching_status_pub = rospy.Publisher('/goal_reaching_status', Bool, queue_size=1)
    ...
    def _init_env_variables(self):
        ...
        """
        # For Info Purposes
        self.cumulated_reward = 0.0 # 重置当前回合的累积奖励
        # Set to false Done, because its calculated asyncronously
        self._episode_done = False #标记当前回合未结束（后续由_is_done判断是否结束）。
        ...
        #获取机器人初始里程计数据，转换为位置（x, y）、姿态（theta，通过四元数转欧拉角）、线速度（u）和角速度（omega），存储在odom_dict_init中。
