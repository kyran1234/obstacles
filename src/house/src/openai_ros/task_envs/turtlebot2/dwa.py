...
## 运动模型：根据当前状态和控制量（线速度v、角速度w）预测下一时刻机器人位置
# Model to determine the expected position of the robot after moving along trajectory
def motion(x, u, dt):
    # motion model
    # x = [x(m), y(m), theta(rad), v(m/s), omega(rad/s)]机器人状态（位置、姿态、速度）
    # u = [v, w]：控制量（线速度、角速度）
    x[2] += u[1] * dt
...
# 计算动态窗口：根据机器人物理约束（最大速度、加速度）确定可行的速度范围
# Determine the dynamic window from robot configurations
def calc_dynamic_window(x, config):
    # Vs：机器人硬件允许的速度范围（最小/最大线速度、最小/最大角速度）
    # Dynamic window from robot specification
    Vs = [config.min_speed, config.max_speed,
          -config.max_yawrate, config.max_yawrate]
    # Vd：基于当前速度和加速度约束的可达速度范围（避免速度突变）（只是范围，还不是集合）
    # Dynamic window from motion model
    Vd = [x[3] - config.max_accel * config.dt, # 线速度下限（当前速度-最大加速度×时间）
          x[3] + config.max_accel * config.dt, # 线速度上限（当前速度+最大加速度×时间）
          x[4] - config.max_dyawrate * config.dt, # 角速度下限
          x[4] + config.max_dyawrate * config.dt] # 角速度上限
    
    # 动态窗口：取Vs和Vd的交集（确保速度在物理可行范围内）（只是范围，还不是集合）
    #  [vmin, vmax, yawrate min, yawrate max]
    dw = [max(Vs[0], Vd[0]), min(Vs[1], Vd[1]), # 最终线速度下限 # 最终线速度上限
          max(Vs[2], Vd[2]), min(Vs[3], Vd[3])]

    return dw

# 计算轨迹：根据当前状态和采样的速度（v, w）预测未来一段时间的轨迹
# Calculate a trajectory sampled across a prediction time
def calc_trajectory(xinit, v, y, config):
 
    x = np.array(xinit)  # 初始状态
    traj = np.array(x)  # 轨迹数组（存储每一步的状态）many motion models stored per trajectory
    time = 0
    while time <= config.predict_time: # 在预测时间内迭代
        # store each motion model along a trajectory
        x = motion(x, [v, y], config.dt) # 用运动模型更新状态
        traj = np.vstack((traj, x)) # 将新状态加入轨迹
        time += config.dt # next sample# 时间累加
...
# Begin DWA calculations# DWA控制主函数：整合动态窗口计算和最优控制量选择
